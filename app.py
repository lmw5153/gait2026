...
def safe_zip_member_name(name: str) -> str:
    """ZIP 내부 경로를 OS 독립적으로 정리한다."""
    return str(name).replace("\\", "/").lstrip("/")



def normalize_trial_id_from_name(filename: str, walking_regex: str = r"^6m[_\- ]*\d*") -> Tuple[Optional[str], str]:
    """
    파일명에서 walking trial ID를 추출한다.

    예:
    - 6m_1.mot      -> 6m_1
    - 6m_1(1).trc   -> 6m_1   # 윈도우 복사본 suffix 제거
    - 6m-2.mot      -> 6m_2
    - TUG_1.mot     -> None, not_walking
    """
    stem = Path(filename).stem.strip()
    stem = re.sub(r"\(\d+\)$", "", stem).strip()
    stem_norm = re.sub(r"[\s\-]+", "_", stem)
    try:
        is_walking = re.search(walking_regex, stem_norm, flags=re.IGNORECASE) is not None
    except re.error:
        is_walking = stem_norm.lower().startswith("6m")
    if not is_walking:
        return None, "not_walking_6m"
    m = re.search(r"(6m(?:_\d+)?)", stem_norm, flags=re.IGNORECASE)
    if not m:
        return "6m", "ok"
...
def infer_subject_from_zip_path(rel_path: str, institution: str, id_regex: str = DEFAULT_ID_REGEX) -> Tuple[str, str]:
    """
    기관별 ZIP 내부 경로에서 subject_id를 추출한다.

    권장 구조:
    - UNI1/6m_1.mot                 # 기관별 ZIP인 경우
    - UNI/UNI1/6m_1.mot             # 통합 ZIP 또는 기관 폴더가 한 번 더 있는 경우
    - OpenCap_DATA/UNI/UNI1/6m_1.mot

    반환: (subject_id, reason)
    """
    rel_path = safe_zip_member_name(rel_path)
    parts = [p for p in rel_path.split("/") if p and not p.startswith(".") and p != "__MACOSX"]
    if len(parts) <= 1:
        return extract_subject_id(Path(rel_path).name, id_regex), "fallback_filename"

    inst_upper = str(institution).upper()
    subject_candidate = None

    # 경로 안에 기관명이 있으면 그 다음 폴더를 subject folder로 사용
    for i, part in enumerate(parts[:-1]):
        if part.upper() == inst_upper and i + 1 < len(parts) - 0:
            subject_candidate = parts[i + 1]
            break

    # 기관별 ZIP에서는 첫 번째 폴더가 보통 subject folder
    if subject_candidate is None:
        # root 폴더명이 OpenCap_DATA 같은 wrapper이고 두 번째가 기관명인 경우 처리
        if len(parts) >= 3 and parts[1].upper() == inst_upper:
            subject_candidate = parts[2]
        else:
            subject_candidate = parts[0]

    subject_candidate = Path(subject_candidate).name.strip()
    # UUH4_1, UNI1_copy 같은 재촬영/복사 폴더는 CRF 매핑을 위해 기관코드+숫자까지만 canonicalize
    m = re.match(rf"^({re.escape(inst_upper)}\d+)", subject_candidate.upper())
    if m:
        return m.group(1), "folder_institution_subject"
    return subject_candidate, "folder_subject"
...
def build_upload_index(
    ...
    """이전 방식: MOT/TRC를 직접 다중 업로드했을 때 파일명에서 subject_id를 추출한다."""
    ...
    for kind, files in (("MOT", mot_files), ("TRC", trc_files)):
        for f in files or []:
            ...
            trial_id, trial_status = normalize_trial_id_from_name(f.name)
            if trial_id is None:
                continue
            time_col = guess_time_col(df) if not df.empty else None
            key = f"DIRECT:{sid}:{trial_id}:{kind}:{f.name}"
            ...
            rows.append(
                {
                    ...
                    "parse_status": trial_status,
                    "subject_source": "filename_regex",
                }
            )
...
def build_institution_zip_upload_index(
    institution_zip_files: Dict[str, object],
    walking_regex: str,
    id_regex: str = DEFAULT_ID_REGEX,
) -> Tuple[pd.DataFrame, Dict[str, ParsedUpload], pd.DataFrame]:
    """
    기관별 ZIP을 읽어 MOT/TRC 파일 index를 만든다.

    중요 설계:
    - MOT와 TRC는 같은 subject 폴더 안에 있어도 확장자로 구분한다.
    - walking은 파일명 trial_id가 6m, 6m_1, 6m_2... 형태인 것만 포함한다.
    - subject_id는 파일명이 아니라 ZIP 내부 폴더명에서 가져온다.
    """
    parsed: Dict[str, ParsedUpload] = {}
    rows = []
    skipped_rows = []

    for institution, zfile in institution_zip_files.items():
        if zfile is None:
            continue
        try:
            zbytes = zfile.getvalue()
            zf = zipfile.ZipFile(io.BytesIO(zbytes))
        except Exception as e:
            skipped_rows.append({"institution": institution, "relative_path": getattr(zfile, "name", ""), "reason": f"zip_open_error: {e}"})
            continue

        for info in zf.infolist():
            rel_path = safe_zip_member_name(info.filename)
            if info.is_dir():
                continue
            path_obj = Path(rel_path)
            fname = path_obj.name
            if not fname or fname.startswith(".") or "__MACOSX" in rel_path:
                continue
            ext = path_obj.suffix.lower().lstrip(".")
            if ext not in {"mot", "trc", "sto", "txt", "csv"}:
                skipped_rows.append({"institution": institution, "relative_path": rel_path, "file_name": fname, "reason": "unsupported_extension"})
                continue

            kind = "MOT" if ext in {"mot", "sto"} else ("TRC" if ext == "trc" else None)
            if kind is None:
                # txt/csv는 이름 안에 mot/trc가 명확할 때만 사용
                low = fname.lower()
                if "mot" in low:
                    kind = "MOT"
                elif "trc" in low:
                    ...
                    skipped_rows.append({"institution": institution, "relative_path": rel_path, "file_name": fname, "reason": "ambiguous_txt_csv"})
                    continue

            trial_id, trial_status = normalize_trial_id_from_name(fname, walking_regex=walking_regex)
            if trial_id is None:
                skipped_rows.append({"institution": institution, "relative_path": rel_path, "file_name": fname, "file_type": kind, "reason": trial_status})
                continue

            subject_id, subject_source = infer_subject_from_zip_path(rel_path, institution=institution, id_regex=id_regex)
            try:
                b = zf.read(info)
                df = parse_opensim_table_cached(b, fname)
                time_col = guess_time_col(df) if not df.empty else None
                n_features = len(numeric_feature_columns(df, time_col)) if not df.empty else 0
                key = f"{institution}:{subject_id}:{trial_id}:{kind}:{rel_path}"
                parsed[key] = ParsedUpload(
                    name=fname,
                ...
                skipped_rows.append({"institution": institution, "relative_path": rel_path, "file_name": fname, "file_type": kind, "reason": f"parse_error: {e}"})

    file_index = pd.DataFrame(rows)
    skipped_df = pd.DataFrame(skipped_rows)
    return file_index, parsed, skipped_df



def make_trial_pair_index(file_index: pd.DataFrame) -> pd.DataFrame:
    """subject-trial 단위로 MOT/TRC pair가 맞는지 진단한다."""
    if file_index.empty:
        return pd.DataFrame()
    gcols = ["institution", "subject_id", "trial_id"]
    pair = (
        file_index.groupby(gcols + ["file_type"])
        .size()
    ...
    for col in ["MOT", "TRC"]:
        if col not in pair.columns:
            pair[col] = 0
    pair["pair_status"] = np.select(
        [
            (pair["MOT"] >= 1) & (pair["TRC"] >= 1),
            (pair["MOT"] >= 1) & (pair["TRC"] == 0),
            (pair["MOT"] == 0) & (pair["TRC"] >= 1),
        ],
        ["paired", "mot_only", "trc_only"],
        default="missing",
    )
    return pair.sort_values(gcols).reset_index(drop=True)
...
# =============================================================================
# CRF 자동 표준화 / 기관별 CRF 병합
# =============================================================================


def _deduplicate_columns(cols: Sequence[object]) -> List[str]:
    out = []
    seen = {}
...
def _detect_crf_header_row(raw: pd.DataFrame) -> int:
    """CRF 엑셀에서 실제 변수명이 있는 행을 자동 탐지한다."""
    best_i, best_score = 0, -1
    for i in range(min(12, len(raw))):
        vals = [str(v) for v in raw.iloc[i].tolist() if not pd.isna(v)]
        joined = " | ".join(vals)
        score = 0
        for token in ["피험자번호", "Session", "Sub Num", "피험자군", "성별", "만 나이", "UPDRS", "6m_1"]:
...
def _read_excel_sheet_auto(file_bytes: bytes, sheet_name: str) -> pd.DataFrame:
    raw = pd.read_excel(io.BytesIO(file_bytes), sheet_name=sheet_name, header=None)
    if raw.empty:
        return pd.DataFrame()
    header_i = _detect_crf_header_row(raw)
    df = pd.read_excel(io.BytesIO(file_bytes), sheet_name=sheet_name, header=header_i)
...
def _score_crf_sheet(sheet_name: str, institution: str) -> int:
    ...
    score = 0
    if any(a.upper() in name for a in aliases):
        score += 5
    if "분석" in sheet_name or "가능" in sheet_name:
        score += 2
    return score



def _find_col_contains(df: pd.DataFrame, patterns: Sequence[str]) -> Optional[str]:
    for pat in patterns:
        for c in df.columns:
...
def _format_subject_id_from_crf_value(value, institution: str) -> str:
    if pd.isna(value):
        return ""
    s = str(value).strip()
    if not s:
    ...
    if re.match(r"^[A-Za-z]+\d+", s):
        return s.upper()
    if re.match(r"^\d+$", s) and institution:
        return f"{institution.upper()}{s}"
    return s



def standardize_crf_dataframe(df: pd.DataFrame, institution: str, source_name: str = "") -> pd.DataFrame:
    """기관별 CRF를 최대한 공통 컬럼으로 표준화한다."""
    if df.empty:
        return df
    out = df.copy()
    out.columns = _deduplicate_columns(out.columns)

    subject_col = _find_col_contains(out, ["피험자번호", "Session Name", "Sub Num", "subject", "id"])
    inst_col = _find_col_contains(out, ["녹화기관", "기관", "institution"])
    group_col = _find_col_contains(out, ["피험자군", "group", "diagnosis"])
    sex_col = _find_col_contains(out, ["성별", "sex"])
    age_col = _find_col_contains(out, ["만 나이", "age"])
    height_col = _find_col_contains(out, ["키", "height"])
    weight_col = _find_col_contains(out, ["체중", "weight"])
    hand_col = _find_col_contains(out, ["주손", "arm dominance", "dominant_hand"])
    foot_col = _find_col_contains(out, ["주발", "leg dominance", "dominant_foot"])
    hy_col = _find_col_contains(out, ["호엔", "HY", "Hoehn"])
    updrs_col = _find_col_contains(out, ["UPDRS"])

    if inst_col is not None:
        out["institution_auto"] = out[inst_col].astype(str).str.strip().replace({"nan": institution})
        out["institution_auto"] = out["institution_auto"].where(out["institution_auto"].str.len() > 0, institution)
    else:
        out["institution_auto"] = institution

    if subject_col is not None:
        out["subject_id_auto"] = [
            _format_subject_id_from_crf_value(v, inst if inst else institution)
            for v, inst in zip(out[subject_col], out["institution_auto"])
        ]
    else:
        out["subject_id_auto"] = ""

    rename_map = {
        "group_auto": group_col,
        "sex_auto": sex_col,
        "age_auto": age_col,
        "height_auto": height_col,
        "weight_auto": weight_col,
        "dominant_hand_auto": hand_col,
        "dominant_foot_auto": foot_col,
        "hy_stage_auto": hy_col,
        "updrs_auto": updrs_col,
    }
    for new_col, old_col in rename_map.items():
        if old_col is not None and old_col in out.columns:
            out[new_col] = out[old_col]
        elif new_col not in out.columns:
            out[new_col] = np.nan

    # 6m trial 상태 컬럼은 원 컬럼명이 6m_1/6m_2/6m_3이면 유지하고, 없으면 유사 컬럼을 복사
    for trial in ["6m_1", "6m_2", "6m_3", "6m"]:
        if trial not in out.columns:
            col = _find_col_contains(out, [trial])
            if col is not None:
                out[trial] = out[col]

    out["crf_source_file"] = source_name
    out["crf_source_institution"] = institution
    out = out[out["subject_id_auto"].astype(str).str.len() > 0].reset_index(drop=True)
    return out



def read_institution_crf_auto(uploaded_file, institution: str) -> pd.DataFrame:
    """기관별 CRF 파일을 자동으로 읽고 표준화한다."""
    ...
    if name.lower().endswith(".csv"):
        ...
        return standardize_crf_dataframe(df, institution=institution, source_name=name)

    sheets = read_excel_sheets_cached(file_bytes)
    # 기관명과 가장 잘 맞는 sheet를 우선 선택
    ranked = sorted(sheets, key=lambda sh: _score_crf_sheet(sh, institution), reverse=True)
    best_df = pd.DataFrame()
    best_score = -1
    for sh in ranked:
        try:
            tmp = _read_excel_sheet_auto(file_bytes, sh)
            std = standardize_crf_dataframe(tmp, institution=institution, source_name=f"{name}::{sh}")
            useful = int(std.get("subject_id_auto", pd.Series(dtype=str)).astype(str).str.len().gt(0).sum())
            score = _score_crf_sheet(sh, institution) * 1000 + useful
            if useful > 0 and score > best_score:
                best_df = std
                best_score = score
        except Exception:
            continue
...
def build_crf_mapping_diagnostics(file_index: pd.DataFrame, crf: pd.DataFrame, subject_col: str) -> pd.DataFrame:
    """파일 index와 CRF가 subject/trial 수준에서 매핑되는지 확인한다."""
    if file_index.empty or crf.empty or subject_col not in crf.columns:
        return pd.DataFrame()
    pair = make_trial_pair_index(file_index)
    crf_tmp = crf.copy()
    crf_tmp["_subject_key"] = crf_tmp[subject_col].astype(str).str.strip().str.upper()
    keep_cols = ["_subject_key"]
    for c in ["institution_auto", "group_auto", "sex_auto", "age_auto", "hy_stage_auto", "updrs_auto", "6m", "6m_1", "6m_2", "6m_3"]:
        if c in crf_tmp.columns and c not in keep_cols:
            keep_cols.append(c)
    crf_small = crf_tmp[keep_cols].drop_duplicates("_subject_key")
    pair["_subject_key"] = pair["subject_id"].astype(str).str.strip().str.upper()
    diag = pair.merge(crf_small, on="_subject_key", how="left")
    diag["crf_matched"] = diag[subject_col if subject_col in diag.columns else "_subject_key"].notna() if subject_col in diag.columns else diag["group_auto"].notna()

    def trial_status(row):
        tid = str(row["trial_id"])
        return row[tid] if tid in row.index else np.nan

    diag["crf_trial_status"] = diag.apply(trial_status, axis=1)
    diag["mapping_status"] = np.select(
        [
            diag["crf_matched"] & diag["pair_status"].eq("paired"),
            diag["crf_matched"] & diag["pair_status"].ne("paired"),
            ~diag["crf_matched"],
        ],
        ["ok", "crf_ok_file_pair_incomplete", "crf_not_matched"],
        default="check",
    )
    return diag.drop(columns=["_subject_key"], errors="ignore")
...
st.sidebar.info("온라인 웹앱에서는 폴더를 그대로 올릴 수 없으므로, 기관별 검사 폴더를 ZIP으로 압축해서 업로드합니다.")
upload_mode = st.sidebar.radio(
    "업로드 방식",
    ["기관별 ZIP + 기관별 CRF", "직접 MOT/TRC 파일 업로드(테스트용)"],
    index=0,
)

st.sidebar.header("2) 매핑 설정")
institution_text = st.sidebar.text_input("기관 코드", value="UNI,UUH,JBH")
institutions = [x.strip().upper() for x in institution_text.split(",") if x.strip()]
id_regex = st.sidebar.text_input("직접 업로드 시 파일명 ID 추출 정규식", value=DEFAULT_ID_REGEX)
walking_regex = st.sidebar.text_input("Walking trial 파일명 정규식", value=r"^6m[_\- ]*\d*")
st.sidebar.caption("기관별 ZIP에서는 subject_id를 파일명이 아니라 ZIP 내부 비식별폴더명에서 추출합니다. MOT/TRC는 같은 폴더에 있어도 확장자로 구분합니다.")
...
if upload_mode == "기관별 ZIP + 기관별 CRF":
    for inst in institutions:
        with st.sidebar.expander(f"{inst} 업로드", expanded=(inst == institutions[0])):
            institution_zip_files[inst] = st.file_uploader(
                f"{inst} 검사 데이터 ZIP",
                type=["zip"],
                key=f"zip_{inst}",
                help="ZIP 내부 구조 예: UNI1/6m_1.mot, UNI1/6m_1.trc 또는 UNI/UNI1/6m_1.mot",
            )
            institution_crf_files[inst] = st.file_uploader(
                f"{inst} CRF 파일",
                type=["xlsx", "xls", "csv"],
                key=f"crf_{inst}",
            )
    ...
    st.sidebar.caption("직접 업로드는 폴더 경로가 보존되지 않으므로 파일명에 subject_id가 반드시 포함되어야 합니다.")
...
skipped_file_index = pd.DataFrame()
trial_pair_index = pd.DataFrame()
crf = pd.DataFrame()

if upload_mode == "기관별 ZIP + 기관별 CRF":
    if any(v is not None for v in institution_zip_files.values()):
        with st.spinner("기관별 ZIP에서 walking MOT/TRC 파일을 파싱하는 중..."):
            file_index, parsed, skipped_file_index = build_institution_zip_upload_index(
                institution_zip_files,
                walking_regex=walking_regex,
                id_regex=id_regex,
            )
    crf_parts = []
    for inst, cf in institution_crf_files.items():
        if cf is not None:
            with st.spinner(f"{inst} CRF를 자동 표준화하는 중..."):
                tmp = read_institution_crf_auto(cf, inst)
                if not tmp.empty:
                    crf_parts.append(tmp)
    if crf_parts:
        crf = pd.concat(crf_parts, axis=0, ignore_index=True)
else:
    ...
    if crf_file is not None:
        if crf_file.name.lower().endswith(".csv"):
            ...
            raw_crf = _read_excel_sheet_auto(crf_file.getvalue(), sheet)
            crf = standardize_crf_dataframe(raw_crf, institution="DIRECT", source_name=f"{crf_file.name}::{sheet}")

if not file_index.empty:
    trial_pair_index = make_trial_pair_index(file_index)
...
with tab_upload:
    ...
    if file_index.empty:
        ...
        st.markdown("#### 업로드 파일 index: walking 6m trial만 포함")
        st.dataframe(file_index, use_container_width=True)

        if not trial_pair_index.empty:
            st.markdown("#### MOT/TRC trial pair 진단")
            st.caption("MOT와 TRC는 같은 비식별폴더 안에서 확장자로 구분하고, 동일한 trial_id끼리 pair 여부를 확인합니다.")
            st.dataframe(trial_pair_index, use_container_width=True)
            c1, c2, c3 = st.columns(3)
            c1.metric("paired trial", int((trial_pair_index["pair_status"] == "paired").sum()))
            c2.metric("MOT only", int((trial_pair_index["pair_status"] == "mot_only").sum()))
            c3.metric("TRC only", int((trial_pair_index["pair_status"] == "trc_only").sum()))

        if not skipped_file_index.empty:
            with st.expander("제외된 파일: 비-walking/TUG/지원하지 않는 확장자/파싱 실패"):
                st.dataframe(skipped_file_index, use_container_width=True)
    ...
    if crf.empty:
        ...
        for cand in ["subject_id_auto", "subject_id", "SubjectID", "ID", "id", "비식별번호"]:
            if cand in crf_cols:
        ...
        default_group_idx = 0
        for cand in ["group_auto", "group", "Group", "diagnosis", "Diagnosis", "군", "그룹", "피험자군"]:
            if cand in crf_cols:
        ...
        crf_tmp = crf.copy()
        crf_tmp[subject_col] = crf_tmp[subject_col].astype(str).str.strip().str.upper()
        if not file_index.empty:
            upload_sids = set(file_index["subject_id"].astype(str).str.strip().str.upper())
            ...
            if len(matched) == 0:
                st.warning("MOT/TRC 비식별폴더 ID와 CRF subject ID가 매칭되지 않습니다. 기관코드/비식별폴더명/CRF ID 컬럼을 확인하세요.")

            mapping_diag = build_crf_mapping_diagnostics(file_index, crf_tmp, subject_col=subject_col)
            if not mapping_diag.empty:
                st.markdown("#### CRF ↔ walking trial 매핑 진단")
                st.caption("subject_id, trial_id, MOT/TRC pair 여부, CRF의 6m trial 상태값을 함께 확인합니다.")
                st.dataframe(mapping_diag, use_container_width=True)
                st.session_state["mapping_diag"] = mapping_diag
        ...
        likely_covs = [c for c in ["age_auto", "sex_auto", "height_auto", "weight_auto", "dominant_hand_auto", "dominant_foot_auto", "age", "sex", "height", "weight", "Age", "Sex", "Height", "Weight"] if c in crf_cols]
...
# =============================================================================
# 최하단 전체 분석 자료 ZIP 다운로드
# =============================================================================

st.divider()
st.subheader("📦 전체 분석 자료 종합 ZIP 다운로드")
st.caption("업로드 index, MOT/TRC pair 진단, CRF 매핑 진단, 전처리 자료, FDA/fPCA 결과, 임상척도 결과, ML 결과를 한 번에 저장합니다.")

all_export_files: Dict[str, pd.DataFrame] = {}
if not file_index.empty:
    all_export_files["00_upload/file_index_walking_trials.csv"] = file_index
if not skipped_file_index.empty:
    all_export_files["00_upload/skipped_nonwalking_or_failed_files.csv"] = skipped_file_index
if not trial_pair_index.empty:
    all_export_files["00_upload/mot_trc_trial_pair_index.csv"] = trial_pair_index
if not crf.empty:
    all_export_files["00_upload/crf_standardized_combined.csv"] = crf
...
settings_df = pd.DataFrame([
    {
        ...
        "walking_regex": walking_regex,
        "id_regex_direct_upload": id_regex,
        "n_grid": n_grid,
        "outlier_percentile": outlier_pct,
        "use_kalman": use_kalman,
        "spline_smoothing": spline_smoothing,
        ...
        "n_perm": n_perm,
        "cv_splits": cv_splits,
        "logistic_l2_c": logistic_c,
        "random_seed": random_seed,
    }
])
all_export_files["00_upload/analysis_settings.csv"] = settings_df

for key, path_name in [
    ("mapping_diag", "00_upload/crf_mapping_diagnostics.csv"),
    ("curve_long", "01_preprocessing/preprocessed_subject_mean_curves_long.csv"),
    ("qc_df", "01_preprocessing/preprocessing_qc_by_trial_feature.csv"),
    ("excluded_df", "01_preprocessing/excluded_outlier_trajectories.csv"),
    ("adjusted_long", "02_fda_fpca/adjusted_curves_long.csv"),
    ("scores_long", "02_fda_fpca/fpca_scores_long.csv"),
    ("scores_wide", "02_fda_fpca/fpca_scores_wide.csv"),
    ("loadings_long", "02_fda_fpca/fpca_loadings_long.csv"),
    ("evr_df", "02_fda_fpca/fpca_explained_variance.csv"),
    ("fpca_tests_df", "02_fda_fpca/fpca_2d_group_tests.csv"),
    ("clinical_corr_df", "03_clinical/fpca_clinical_correlations.csv"),
    ("ml_fold_df", "04_ml/ml_fold_metrics.csv"),
    ("ml_oof_metrics_df", "04_ml/ml_oof_metrics.csv"),
    ("ml_oof_df", "04_ml/ml_oof_predictions.csv"),
    ("ml_coef_summary", "04_ml/ml_logistic_coefficients.csv"),
]:
    df = st.session_state.get(key, pd.DataFrame())
    if isinstance(df, pd.DataFrame) and not df.empty:
        all_export_files[path_name] = df

if all_export_files:
    st.download_button(
        "전체 분석 자료 ZIP 다운로드",
        data=dataframe_to_zip_bytes(all_export_files),
        file_name="opencap_gait_all_analysis_results.zip",
        mime="application/zip",
        type="primary",
    )
else:
    st.info("분석 자료가 생성되면 전체 ZIP 다운로드 버튼이 활성화됩니다.")
