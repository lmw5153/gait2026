...
st.caption("Analysis-only 버전: 원본 MOT/TRC 파싱은 분리하고, 전처리된 gait curve + CRF만 업로드해 분석합니다.")
...
with st.sidebar:
    st.header("1) 입력 자료")
    gait_file = st.file_uploader("전처리된 gait curve 데이터 CSV/XLSX/ZIP", type=["csv", "xlsx", "xls", "zip"])
    st.caption("필수 컬럼: subject_id, feature, grid_pct, value")

    st.subheader("기관별 CRF")
    crf_files = {}
    for inst in INSTITUTIONS:
        crf_files[inst] = st.file_uploader(f"{inst} CRF", type=["xlsx", "xls", "csv"], key=f"crf_{inst}")
...
if load_clicked:
    try:
        gait_raw = read_preprocessed_gait_file(gait_file)
        gait_long, gait_errors = standardize_gait_long(gait_raw)
        ...
        if gait_errors:
            ...
            st.success("데이터 로드/매핑 준비 완료")
    except Exception as e:
        st.session_state["loaded"] = False
        st.error(f"데이터 로드 실패: {e}")

if not st.session_state.get("loaded"):
    st.info("전처리된 gait curve 데이터와 CRF를 업로드한 뒤, 사이드바의 `① 데이터 로드/매핑`을 눌러주세요.")
